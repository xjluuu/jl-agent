...
- **通用 Risk Agent 一票否决**：`risk_veto_research` 可以用于任何拿不准的决策。多个子 Agent 先从不同立场积极讨论，最后由独立 Risk Agent 做一票否决；只要触发红线，就返回 `veto: true`。
...
- **研究模板**：`--research-templates` 内置通用决策、股票、尽调、内部项目四类研究框架，并带风险规则。
...
```text
当你不确定一件事要不要做，可以用 risk_veto_research。
多个子 Agent 会积极讨论：
1. 为什么应该推进
2. 为什么应该停止或推迟
3. 证据、假设和缺失数据
...
Risk Agent 一票否决是通用决策风险闸门。金融/股市相关能力只用于研究辅助。JL-Agent 不提供投资建议，不执行交易，也不能替代公司合规审查。
...
Risk Agent 的一票否决是通用研究和决策控制机制。金融/股市相关能力用于整理和分析信息，不是投资建议，也不是自动交易系统。
